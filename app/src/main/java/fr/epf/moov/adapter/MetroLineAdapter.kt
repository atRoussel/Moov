...
class MetroLineAdapter(val metroLines: List<MetroLine>) :
    RecyclerView.Adapter<MetroLineAdapter.MetroLineViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MetroLineViewHolder, position: Int) {
        ...
        holder.metroLineView.setOnClickListener {
            if (metro.code == "C" || metro.code == "D") {
                Toast.makeText(this.context, "Indisponible pour le moment", Toast.LENGTH_SHORT)
                    .show()
            } else {
